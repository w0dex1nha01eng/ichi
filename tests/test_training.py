...
def create_serializable_episode(sample_count: int, map_seed: int) -> ExpertEpisode:
    """Create a deterministic episode used by the end-to-end training test."""
    sample_axis = np.linspace(-1.0, 1.0, sample_count, dtype=np.float32)
    ...
    actions = np.column_stack(
        [
            0.5 + 0.2 * sample_axis,
            0.3 * sample_axis - 0.1 * np.sin(sample_axis),
        ]
    ).astype(np.float32)
...
def test_one_training_epoch_updates_model_parameters() -> None:
    ...
    # Arrange
    set_seed(5)
    model = BCOneStep(obs_dim=4, obs_horizon=2, hidden_dim=32)
...
def test_model_can_overfit_small_deterministic_dataset() -> None:
    ...
    # Arrange
    set_seed(7)
    model = BCOneStep(obs_dim=4, obs_horizon=2, hidden_dim=64)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-2)
...
def test_checkpoint_round_trip_restores_predictions_and_metadata(
    ...
    # Arrange
    set_seed(11)
    source_model = BCOneStep(obs_dim=4, obs_horizon=2, hidden_dim=16)
    ...
    restored_model = BCOneStep(obs_dim=4, obs_horizon=2, hidden_dim=16)
...
def test_train_bc_runs_end_to_end_and_saves_best_checkpoint(tmp_path: Path) -> None:
    ...
    # Assert
    assert isinstance(model, BCOneStep)
