...
class BC_Chunk(nn.Module):
    ...
    def __init__(self,obs_dim: int,obs_horizon: int = 2,pred_horizon: int = 16,hidden_dim: int = 256) -> None:
        super().__init__()
        ...
        self.network = nn.Sequential(nn.Linear(input_dim,hidden_dim),nn.ReLU(),nn.Linear(hidden_dim,hidden_dim),nn.ReLU(),nn.Linear(hidden_dim,output_dim))
