...
def train_bc_epoch(model: BC_1,dataloader: DataLoader,optimizer: torch.optim.Optimizer,device: torch.device | str) -> float:
...
def save_bc_checkpoint(checkpoint_path,model: BC_1,optimizer: torch.optim.Optimizer,normalizer: Normalizer,epoch: int,metrics: dict[str, float]) -> None:
    checkpoint_path = Path(checkpoint_path)
    checkpoint_path.parent.mkdir(parents=True,exist_ok=True)
    checkpoint = {"model_state_dict": model.state_dict(),"optimizer_state_dict": optimizer.state_dict(),"model_config": {"obs_dim": model.obs_dim,"obs_horizon": model.obs_horizon,"hidden_dim": model.hidden_dim},"normalizer": normalizer.state_dict(),"epoch": int(epoch),"metrics": {name: float(value) for name, value in metrics.items()}}
    torch.save(checkpoint,checkpoint_path)


def load_bc_checkpoint(checkpoint_path,model: BC_1,optimizer: torch.optim.Optimizer | None = None,device: torch.device | str = "cpu") -> dict:
    ...
    expected_config = {"obs_dim": model.obs_dim,"obs_horizon": model.obs_horizon,"hidden_dim": model.hidden_dim}
...
def train_bc(config: dict) -> tuple[BC_1, Normalizer, list[dict[str, float]]]:
    ...
    for epoch in range(1,epochs + 1):
        ...
        print(f"epoch {epoch:03d} | train_loss {train_loss:.6f} | validation_loss {validation_loss:.6f}")
        if validation_loss < best_validation_loss:
            best_validation_loss = validation_loss
            save_bc_checkpoint(output_config["checkpoint_path"],model,optimizer,train_dataset.normalizer,epoch,metrics)
