...
def _prepare_chunk_batch(
    batch, device: torch.device, pred_horizon: int) -> tuple[torch.Tensor, torch.Tensor]:
    if "obs" not in batch or "action" not in batch:
        raise KeyError("batch must contain obs and action")
    observations = torch.as_tensor(batch["obs"], dtype=torch.float32, device=device)
    actions = torch.as_tensor(batch["action"], dtype=torch.float32, device=device)
...
def train_bc_chunk(config: dict) -> tuple[BC_Chunk, Normalizer, list[dict[str, float]]]:
    ...
    obs_dim = int(train_dataset.observations.shape[1])
    model = BC_Chunk(obs_dim, obs_horizon, pred_horizon, 
                     int(model_config.get("hidden_dim", 256))).to(device)
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=float(training_config.get("learning_rate", 1e-3)),
        weight_decay=float(training_config.get("weight_decay", 0.0)),
    ...
    best_validation_loss = float("inf")
    for epoch in range(1, epochs + 1):
        train_loss = train_bc_chunk_epoch(model, train_loader, optimizer, device)
...
def main() -> None:
    ...
    model_type = config["model"].get("type", "BC_1")
    if model_type == "BC_Chunk":
        train_bc_chunk(config)
    elif model_type == "BC_1":
        train_bc(config)
    else:
