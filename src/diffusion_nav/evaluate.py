...
from .models import BC_1, BC_Chunk
...
class BCPolicy:
    ...
    def __init__(self,model: BC_1,normalizer: Normalizer,action_low,action_high,device: torch.device | str = "cpu") -> None:
        self.model = model
    ...
    def act(self,observation_history) -> np.ndarray:
        ...
        normalized_action = self.model(observation_tensor).squeeze(0).cpu().numpy()
...
class BCChunkPolicy:
    ...
    def act(self, observation_history) -> np.ndarray:
        ...
        observation_tensor = torch.as_tensor(
            normalized_observation, dtype=torch.float32, device=self.device
        ).unsqueeze(0)
...
def run_policy_episode(env: NavigationEnv,policy: BCPolicy) -> EpisodeResult:
    ...
    return EpisodeResult(np.stack(observations).astype(np.float32),np.stack(actions).astype(np.float32),np.stack(states).astype(np.float32),np.asarray(rewards,dtype=np.float32),success,collision,bool(truncated))


def run_chunk_policy_episode(env: NavigationEnv, policy: BCChunkPolicy, action_horizon: int = 1) -> EpisodeResult:
    if (
        not isinstance(action_horizon, int)
        or isinstance(action_horizon, bool)
        or not 1 <= action_horizon <= policy.model.pred_horizon
    ):
...
def load_bc_policy(checkpoint_path,action_low,action_high,device: torch.device | str = "auto") -> BCPolicy:
    ...
    model = BC_1(**checkpoint["model_config"])
    model.load_state_dict(checkpoint["model_state_dict"])
    normalizer = Normalizer.from_state_dict(checkpoint["normalizer"])
    return BCPolicy(model,normalizer,action_low,action_high,device)
